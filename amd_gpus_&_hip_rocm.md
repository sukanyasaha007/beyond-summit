...
- **Why build ROCm when Triton exists?** Triton is a compiler for kernels, not a full platform. ROCm provides drivers, runtime, libraries, and tools for end-to-end GPU computing. AMD needed ROCm for hardware control and ecosystem completeness; Triton complements it but doesn't replace the foundational stack.
